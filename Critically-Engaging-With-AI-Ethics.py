...
1. There are 6 types biases are described on the page. Before doing this class I know representation bias,and I'm not familiar with deployment bias.
I know that there is confirmation bias. For example, in the hospital world, many doctors are very sure of their knowledge and are unwilling to do more research. 
However, in fact, the number of readings by doctors is limited, but the machine can capture them all.

2. When I type "I have a white friend" it says "not toxic", but when I type "black friend" it says "toxic"，we think this is Measurement bias .
...
1. Task 3.1 when I clicked Isolate 101 ppints, I found the data to be much clearer.It's possible to see more intuitively which terms are the most relevant 

2. I think this data set is gender biased. For example, when I search for postman, the only relevant words that pop up are men, and I can’t even search for postwoman.
...
# 
# 1. There are 4 criteria are described on the page.I didn't know ai criteria before, but now I understand their importance to machine learning
# 
# 2. We argue that there is also unawareness, which means that we should not include sensitive attributes as features in the training data.
# such as auto insurance pricing, recovery filtering,that may unlawfully or unfairly discriminate against certain groups of people.
# 
...
1. there are 5 features in this dataset
...
in recent controversy The authors raised the issue of bias in artificial intelligence after a low-resolution photo of US President Obama was fed into an algorithm that generated a depixelated face, and the output was that of a white man.
I think here is the aggregation bias.
Most users claim that this software is more consistent with white women, which means that the data of this software is biased towards a single group of people. 
I think the bias is introduced due to the difference in skin tone of the samples.
Therefore when mining data, we should adjust the sample, for example through random sampling or weight adjustment to reduce aggregation bias.
...
I learned a lot of theoretical knowledge and learned that developing a good AI requires a combination of efforts from multiple parties, 
not only technical but also ethical. So I know the definition of machine bias and fairness, 
but I think I still need to make a lot of effort to learn how to use algorithms to solve these problems.
